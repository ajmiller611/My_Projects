...
public class CheckingAccountCard extends JPanel implements ActionListener{
	...
	public CheckingAccountCard(JPanel panel) {
		cardHolderPanel = panel;
		cardHolderPanel.add(createCheckingAccountCard(), "Checking Account");
	}
	
	//Creates GUI for checking account card.
	private JPanel createCheckingAccountCard() {
		JPanel checkingAccountCard = new JPanel(new GridBagLayout());
		GridBagConstraints constraints = new GridBagConstraints();
		
		//The checking account label font is enlarged as it is used as the title of the panel.
	...
	public void actionPerformed(ActionEvent e) {
		...
		if ("DepositIntoChecking".equals(e.getActionCommand())) {
			try {
				...
				checkingAccountBalanceLabel.setText("$" + String.format("%.2f", ATMApplication.currentUser.getCheckingAccount().getBalance()));
