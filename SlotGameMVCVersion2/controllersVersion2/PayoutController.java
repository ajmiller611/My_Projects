...
public class PayoutController {
	...
	public int calculatePayout(SymbolEnum[][] board) {
		...
		//Loop through all paylines to calculate winnings of each payline
		for(Payline payline: paylineList) {
			if(payline.isActive()) {
				//Pass the payline array to calculate winnings on that payline
	...
	private int calculateWinningsOfPayline(SymbolEnum[] paylineArray) {
		...
		/*
		 * Payline array contains SymbolEnum objects on that payline.
		 * Loop through the payline array to check for matching objects
		 * and increment numberOfSameSymbols variable if matching
		 */
		for(int a = 1; a < 5; a++ ) {
		...
		/*
		 * If numberOfSameSymbols variable is equal or greater than 3 then
		 * the the user won on the payline so calculate the winnings.
		 */
